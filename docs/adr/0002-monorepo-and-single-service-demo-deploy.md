...
BankOps Mission Control will keep the React app, stream server, simulator, and shared protocol code
in one pnpm workspace monorepo. The first public deployment should be a single long-running Node
service that serves the built SPA and the `/stream` WebSocket endpoint from the same origin, with a
split static-web plus stream-server deploy left as a later option. This favors implementation speed,
clear local development, and an honest realtime architecture over serverless convenience.
...
server should expose `/healthz` and `/stream` and should be able to serve the built SPA for demo
deploys.
