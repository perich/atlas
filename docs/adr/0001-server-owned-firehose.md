...
aggregate state, and control messages for rate and performance telemetry updates. This keeps the demo
...
The contracts package should be introduced early so the server, `ops-tape.worker.ts`, and tests
share the same frame definitions. React components should consume coalesced snapshots rather than
individual events, so stream rate is not coupled to React commit rate.
